...
public final class PinnedCertificateDelegate: NSObject, URLSessionDelegate {
    ...
    public func urlSession(_: URLSession, didReceive challenge: URLAuthenticationChallenge, completionHandler: @escaping (URLSession.AuthChallengeDisposition, URLCredential?) -> Void) {
        ...
        guard let serverTrust = challenge.protectionSpace.serverTrust, let presentedCertificate = SecTrustGetCertificateAtIndex(serverTrust, 0) else {
            ...
            logger.info("SSL certificate trust for the challenge protection space was not found at index 0.")
            completionHandler(.cancelAuthenticationChallenge, nil)
            return
        }
        ...
        if presentedCertificateData.isEqual(to: certificateData) {
