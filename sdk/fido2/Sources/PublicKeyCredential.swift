...
public struct PublicKeyCredentialCreationOptions: Codable {
    ...
    /// - parameter rp: Represents relying party attributes provided when a credential is created.
    /// - parameter user: Represents user account information provided when a credential is created.
    /// - parameter challenge: The challenge to be used for generating the newly created credential’s attestation object.
    /// - parameter pubKeyCredParams: Represents additional options provided when a credential is created.
    /// - parameter timeout: The time in milliseconds, that the caller is willing to wait for the call to complete.  Default is 30000 milliseconds.
    /// - parameter excludeCredentials: Represents credential parameters to be used for FIDO2 registration or authentication.
    /// - parameter authenticatorSelection: Represents configuration items related to the authenticator, which are specified by the WebAuthn relying party.
    /// - parameter attestation: Represents credential passing preferences, which are used by the WebAuthn relying party when the credential is created.
...
public struct PublicKeyCredentialRpEntity: Codable {
    ...
    /// - parameter id: The unique identifier of a relying party.
    /// - parameter name: The relying party name.
    /// - parameter icon: A URL that refers to the relying party icon.
...
public struct PublicKeyCredentialUserEntity: Codable {
    ...
    /// - parameter id: The unique identifier of a user.
    /// - parameter displayName: The display name of the user.
    /// - parameter name: The name of the user.
    /// - parameter icon: A URL that refers to the users icon.
...
public struct PublicKeyCredentialDescriptor: Codable {
    ...
    /// - parameter id: The credential ID of the public key credential the caller is referring to.
    /// - parameter transports: An array of transport type that can communicate with the client.
