...
public struct AuthenticatorSelectionCriteria: Codable {
    ...
    /// - parameter authenticatorAttachment: An enumeration that describes an authenticators' attachment modalities. Relying parties use this to express a preferred authenticator attachment modality.  ``AuthenticatorAttachment.platform``.
    /// - parameter requireResidentKey: A flag for backwards compatibility with WebAuthn Level 1.  Default is **false**.
    /// - parameter userVerification: Describes the Relying Party's requirements regarding user verification for the create() operation.  Default is `UserVerificationRequirement.preferred`.
