...
internal class CBORReader {
    ...
    func readAny() -> Optional<Any> {
        ...
        if v1 >= 0 && v1 <= 27 {
            ...
            // Support nested-map only when its key is string
    ...
    // Used for attestation operations
    func readStringKeyMap() -> Optional<[String: Any]> {
    ...
    // Used for COSE key
    func readIntKeyMap() -> Optional<[Int64: Any]> {
        guard let b1 = self.nextByte() else {
            return nil
        }
    ...
    func readDouble() -> Optional<Double> {
        guard let b1 = self.readByte() else {
            return nil
        }
    ...
    func readByteString() -> Optional<[UInt8]> {
        guard let b1 = self.nextByte() else {
            return nil
        }
    ...
    func readString() -> Optional<String> {
        guard let b1 = self.nextByte() else {
            return nil
        }
    ...
    // Optional is invalid for result
    func readNull() -> Optional<()> {
        guard let b1 =  self.readByte() else {
            return nil
        }
    ...
    func readNumber() -> Optional<Int64> {
        guard let b1 =  self.readByte() else {
            return nil
        }
...
internal class CBORWriter {
    ...
    // Any must be Int64 | String | Float | Double | Bool
      func putArray(_ values: [Any]) -> CBORWriter {
        ...
        values.forEach {
            if $0 is Int64 {
               ...
               _ = self.putBool($0 as! Bool)
                ...
                fatalError("Value type unsupported.")
            }
        }
        return self
    }
    ...
    func putStringKeyMap(_ values: [String: Any]) -> CBORWriter {
        var bytes = composePositive(UInt64(values.count))
        ...
        for (key, value) in values {
            ...
            if value is Int64 {
                ...
                _ = self.putStringKeyMap(value as! [String: Any])
            }
            else if value is [Any] {
                _ = self.putArray(value as! [Any])
            }
            else {
                fatalError("Value type \(value) unsupported.")
            }
        }
        return self
    }
    ...
    // for COSE_Key format
    func putIntKeyMap(_ values: [Int: Any]) -> CBORWriter {
        var bytes = composePositive(UInt64(values.count))
        bytes[0] = bytes[0] | CBORBits.mapHeader
        self.result.append(contentsOf: bytes)
        for (key, value) in values {
    ...
    func startArray() -> CBORWriter {
        self.result.append(CBORBits.indefiniteArrayBits)
        return self
    }

    func startMap() -> CBORWriter {
    ...
    func putString(_ value: String) -> CBORWriter {
    ...
    func putByteString(_ value: [UInt8]) -> CBORWriter {
    ...
    func putFloat(_ value: Float) -> CBORWriter {
        var fv: Float = value
    ...
    func putDouble(_ value: Double) -> CBORWriter {
        var dv: Double = value
    ...
    func putBool(_ value: Bool) -> CBORWriter {
        if value {
    ...
    func putNull() -> CBORWriter {
        result.append(CBORBits.nullBits)
        return self
    }

    func putNumber(_ value: Int64) -> CBORWriter {
        if value >= 0 {
    ...
    func composeNegative(_ value: Int64) -> [UInt8] {
        assert(value < 0)
    ...
    func composePositive(_ value: UInt64) -> [UInt8] {
        if value >= 0 && value <= 23 {
            return [UInt8(value)]
        }
            ...
            fatalError("Number exceeds type range.")
        }
    }
}
