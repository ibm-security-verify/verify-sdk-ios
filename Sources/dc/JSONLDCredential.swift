...
/// - issued (issuer)
/// - stored (holder)
public struct JSONLDCredential: CredentialDescriptor {
    public let id: String
    public let role: CredentialRole
    public let state: CredentialState
    public let issuerDid: DID
    public let format: CredentialFormat = .jsonld
...
extension JSONLDCredential: Codable {
    // MARK: Enums
    ...
    public init(from decoder: any Decoder) throws {
        ...
        if container.contains(.jsonRepresentation) {
            ...
            // Resolve the document types.
            if let item = data["type"], let type = item.value as? [String] {
                documentTypes = type
            }
